...
export default function  Home () {
    ...
    const displayQuestion = (tag) => {
        ...
        if(session.session !== undefined && session.session !== null){
            let question = session.session.session_steps[2].session_step_backgrounds[2].tool_session_answers.filter((question) => question.tool.name == tag )[0]
    ...
    if(session && session.session){
        return (
            <>
                <div style={{width:'100vw',height:'100vh',background: 'rgb(176 127 73)'}} >
...
dans <span style={{fontWeight: 'bold'}}>{session.session?.session_steps[2]?.step.name}</span></span>
                    ...
                    >
                        ...
                        <path
                            opacity="0" id="malette" className="cursor" onClick={()=>displayQuestion('noyade_victime')}
                            d="M566.233 1871.512l3.677-444.898 566.234 3.677-22.061 455.928z"
                        ></path>
                        <path
                            opacity="0" id="bouee" className="cursor" onClick={()=>displayQuestion('noyade')}
                            d="M352.977 1908.28l-121.336-25.738-110.305-628.74 235.317 14.708 102.952 529.465-36.768 73.537z"
                        ></path>
                        <path
                            opacity="0" id="surf" className="cursor" onClick={()=>displayQuestion('accidents_surf')}
                            d="M2143.598 1893.573l246.348 14.707 132.366-529.465 22.061-911.856-55.152-169.135-62.506 66.183-290.47-7.354-117.66 558.88 36.769 463.282 18.384 316.208z"
                        ></path>
                        <image
                            width="350.819"
                            height="410.953"
